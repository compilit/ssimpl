...
You start with a single seed (often from a [BIP39](#bip39) mnemonic).
...
dead end. Currently, most (if not all) authorities don't have any way to issue a [DID](#did). So what SSIMPL does, is
reversing
...
SSIMPL promotes the use of [Multibase encoding](#multibase-encoding), which means that even the public key part of the
